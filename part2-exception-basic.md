...
1. Attachment: 例外處理程序可附加到哪些受保護的程式區塊
    ...
    1. 類別: BETA, Ada95等
    1. 例外: BETA
1. Resolution: 又稱為handler binding, 針對一個例外, 如何找到可以處理它的例外處理程序(handler)
    1. 靜態範圍: 從程式碼中可直接觀察, 如try-catch
    1. 動態範圍: 透過call chain的關係才能決定誰來處理, 如catch後又拋出, 但在該方法中不知道拋出後最後會由誰處理
    1. 半動態範圍: 本地端可以處理時就用靜態範圍, 找不到處理程序時就透過動態範圍來處理(大部分的程式語言採用此方法)
    1. 堆疊展開(stack unwinding): 程式執行的呼叫順序形成一個call chain, 例外會沿著call chain一層一層往上找handler
    1. 堆疊切割(stack cutting): 把例外處理程序放到list中, 發生例外時從中尋找合適的handler(沒有階層關係)
1. Continuation: 又稱為exception model(例外模型), 例外發生後, 程式的控制流程該如何進行
    1. 終止模型: 發生例外後程式控制權不會返回發生例外的地方, Java, C++均屬此類
    1. 恢復模型: 執行完handler後會繼續執行程式碼, 如VB
    1. 重試模型: 執行完例外處理程序後會先終止例外發生的區域, 接著重新執行一次整個區塊, 如Ruby
1. Cleanup: 無論是否發生例外, 如何讓程式清理資源(如釋放記憶體)以維持在正確狀態
    1. 外顯傳遞: 例外往外傳遞後, 系統會直接呼叫資源清理程式碼, 如C++
    1. 特定構造: 採用特定構造以確保無論是否有例外皆清理, 如Java中的finally
    1. 自動清理: 系統自動清理, 不需要管資源試放的問題, 目前沒有主流程式語言可做到
1. Reliability Check: 程式語言對於不正確使用例外處理機制的檢查支援
    1. 靜態檢查: 編譯時檢查, 如Java會在編譯時檢查checked exception是否有宣告
    1. 動態檢查: 執行時檢查
1. Concurrency: 若程式語言的例外處理有考慮到並行, 則允許同時間可產生多個例外 
    1. 不支援: C++
    1. 部分支援: 可在多執行緒中丟出例外, 但handler的解析需要自行開發, 且系統不支援atomic action(不可分割動作)
    1. 完整支援: 程式語言可指定那些函數屬於不可分割動作, 當執行失敗時例外處理機制會確保這些不可分割動作的狀態完整性(目前尚未出現這種程式語言)
    * Java屬於部分支援, 例外可由不同的執行緒產生, 但執行緒所產生的例外並不會傳遞到其他執行緒中
## 4. 例外處理vs容錯設計
* 例外處理(exception handling): 負責處理component fault(可預期錯誤狀況)
* 容錯設計(fault-tolerant programming): 負責處理design fault(不可預期的錯誤狀況)
* 一般來說, 容錯設計成本更高, 通常使用在需要非常高強健度的應用系統中, 讓程式即使有錯也要能正常運行
